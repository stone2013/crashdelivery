...
6. `GET /rooms` lists available V0.7.5 rooms. `POST /rooms/{code}` registers a room and returns an ephemeral host token; authenticated `PUT` heartbeats update occupancy and authenticated `DELETE` removes the listing. The token is held in page memory and its hash is stored in KV.
